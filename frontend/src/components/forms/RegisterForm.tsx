...
export function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await register({ id: 0, nome, email, senha, telefone, tipo });
      ...
      setErrorMsg(error.response?.data?.error || 'Erro ao fazer cadastro');
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-[#1F1F1F] to-[#2A2A2A] px-4 overflow-hidden">
      <div className="z-10 bg-[#121212] p-8 rounded-xl shadow-2xl w-full max-w-md text-white">
        <h2 className="text-3xl font-bold text-center text-blue-500 mb-6">Crie sua conta</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <select
            ...
            className="w-full px-4 py-2 bg-[#1F1F1F] border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600 text-white"
          >
            <option disabled hidden className='border-gray-600'>Tipo</option>
        ...
        <p className="text-center text-sm mt-6 text-gray-400">
          Já tem conta?{' '}
          <a href="/login" className="text-blue-500 font-medium hover:underline">Entrar</a>
        </p>
      </div>
